...
def get_rank_data(records, top_n=5):
    ...
    for r in records:
        p = r['name']
        # 스코어를 숫자로 강제 변환 (문자열 정렬 오류 방지)
        current_score = int(r['score'])
        if p not in best_per_player or current_score < best_per_player[p]['score']:
...
for row in rows:
    ...
    if date_match and game_match:
        ...
        if start_of_month <= g_date <= end_of_month:
            raw_candidates.append((game_match.group(1).strip(), game_match.group(2).strip(), date_match.group(1), g_date))

for gserial, ccid, date_str, dt_obj in raw_candidates:
    ...
    for i in range(1, 5):
        ...
        try:
            ...
            record = {"name": clean_name, "score": diff, "course": members.get("cc", "알수없음"), "date": date_str}
